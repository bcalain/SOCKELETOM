...
*/
/**
   * e_interfaz_datos_txt_mono_string2 script
   * @version 0.0.1
...
//si la gestion es acertada retornara un string, este strin es el valor, en el par llave:valor resultado condicionado por los parametros utilizados, de lo contrario retornara NULL . 
//esta interfaz necesita la variable de nombre $Ls_pathfinal_datos que se hereda de la entidad EEoInterfazDatos en el include que hace de este fichero parser, y como valor el path o camino del fichero txt que se debe parsear  
	...
	if( !empty( $La_datos_result[0] ) )
		{
			return $La_datos_result[0][1] ; 
		}
	else 
		{ 
			trigger_error('No se ha gestionado el string objeto de la gesti&oacute;n del parser e_interfaz_datos_txt_mono_string2 ', E_USER_WARNING );
		}	
